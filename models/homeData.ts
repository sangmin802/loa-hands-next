...
export default class HomeData {
  ...
  setCalendar(raw: Element) {
    ...
    const today: string = `${_.getFullYear()}-${this.zero(
      _.getMonth() + 1
    )}-${this.zero(_.getDate())}`;
    this.calendar = timerList.reduce(
      (prev, next) => {
        const name: string = next.getElementsByClassName("npcname")[0]
          .textContent;
        const target: object = calendarIsland.find(
          (cal: object) => cal["name"] === name
        );
